...
- Check **permission codes** (`user.invite`, `course.community.read`, …), never role name strings. Codes live in `src/permissions.ts` and must match `migrations/V2__identity_and_access.sql`.
...
- Put reusable logic in `src/helpers/` when it is shared. Keep one controller per area. Add services only when implementing a TODO.
